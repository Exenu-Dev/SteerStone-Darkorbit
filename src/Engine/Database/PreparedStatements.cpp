...
namespace SteerStone { namespace Core { namespace Database {
    ...
    uint32 PreparedStatements::SetUp(std::string const p_Username, std::string const p_Password, uint32 const p_Port, std::string const p_Host, 
    ...
    PreparedStatement * PreparedStatements::ObtainPrepareStatement()
    {
        std::unique_lock<std::mutex> l_Guard(m_Mutex);
